        ...
        for c in self.courses:
            ...
            for d in range(DAYS):
                for s in range(SLOTS_PER_DAY - duration + 1):
                    ...
                    # --- E. 系主任週二禁排 ---
                    if c.teacher.is_director and d == self.setting.director_off_day and any(s_in_d in self.dir_off_slots for s_in_d in slots_in_day):
                        continue
            ...
            for cv in course_vars.values():
                ...
                for class_obj in cv['course'].classes:
                    final_res.append({
                        ...
                        "teacher_name": cv['course'].teacher.name,
                        "room_name": room_name,
